...
<li>“<b>koigawa</b>” contains codes, scripts,  and requirements.txt file used to build a static website and SageMaker application that incorporates scripts within “cdebski” and “jshumway” folders. <b>lambda_app</b> file contains content you need to make a Lambda function using Docker image and <b>nlp_sagemaker_app</b> does the same except for SageMaker inference endpoint instead of Lambda. <b>static_site</b> contains code you need to create website that will act as the main interface. For deployment instructions, please refer to "How to deploy the tool to AWS" section. Lastly, there is a miscellaenous folder called <b>report</b> that contains ipynb for one of the visualizations in the report.</li>
</ul>
...
1. Data collection:<br>&ensp;a) __Data_extraction_gtab.ipynb__ contains the code necessary to extract data for any keyword for any region.
   <br>&ensp;b) All data we leveraged is available in the file __all_merged_data.csv__ and each of the following notebooks begin with importing this data and splitting it directly into train/dev/test partitions. 

3. Hyperparameter tuning and model training:
   <br>&ensp;a) The final trained model is available as __lstm_40_epochs.keras__ alternatively you can follow our workflow through __hyper_parameter_tuning.ipynb__, __hyper_parameter_tuning_viz.ipynb__ and __model_training.ipynb__
   
5. Seasonality:
   <br>&ensp;a) Lastly you can explore seasonality for any keyword via fourier transform with the notebook __fourier_transform_seasonality.ipynb__
...
During this project, we have leveraged Generative AI technology in the form of ChatGPT. While no parts of the code was createdly entirely by ChatGPT, we have used to identify appropriate functions, syntax and so on. In addition to that, there are sections of code which were copied entirely or partially copied from pre-existing AWS code repositories. In those cases, source and license are mentioned on top of the file to indicate this. Lastly, because the underlying data we are using is from Google Trends, we fully acknowledge that the data belongs to Google and that we, as well as any users, fully adhere to [Google’s terms of service](https://policies.google.com/terms?hl=en-US).
