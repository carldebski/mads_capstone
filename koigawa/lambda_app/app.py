    ...
    if event["httpMethod"] == "POST":
        ...
        try:
            ...
            try:
                ...
                print(response)
                response_list = (", ").join(
                    response["Body"].read().decode("utf-8").split(";")[:3]
                )
        ...
        df = pd.read_csv(
            "s3://mads-siads699-capstone-cloud9/data/combined_forecast_df.csv"
        )
        ...
        df_fourier = pd.read_csv(
            "s3://mads-siads699-capstone-cloud9/data/df_fourier.csv"
        )
        df_fourier_dom_cycle = pd.read_csv(
            "s3://mads-siads699-capstone-cloud9/data/df_fourier_dom_cycle.csv"
        )
        ...
        chart = (
            ...
            .encode(
                x=alt.X(
                    "date:T", axis=alt.Axis(format="%Y-%b", labelAngle=-90), title=None
                ),
                y=alt.Y(
                    "value:Q",
                    title="% of Total",
            ...
            .transform_filter((datum.is_prediction == "N"))
        ...
        chart_four = (
            alt.Chart(df_fourier_dom_cycle)
            .mark_text(align="left", baseline="middle", size=20)
            .encode(
                text="text",
        ...
        with alt.themes.enable("fivethirtyeight"):
            chart_json = (
                alt.vconcat(chart, chart_two, chart_three, chart_four)
...
def get_single_keyword_trend_data_gtab(keyword, region="US"):
    ...
    try:
        ...
        print(
            "Querying keyword: %s in region: %s for the period %s"
            % (keyword, region, time_period)
        )
...
def create_forecast_data(keywords, region="US"):
    ...
    ]  # Clean whitespace and ensure each is a string
    region = region.strip()
    ...
    for keyword in keywords:
        ...
        if trend_data is not None:
            if combined_trend_data.empty:
                ...
                combined_trend_data = pd.merge(
                    combined_trend_data, trend_data, on="date", how="outer"
                )

    combined_trend_data.to_csv(
        "s3://mads-siads699-capstone-cloud9/data/combined_trend_data.csv", index=False
    )


def predict_future(model, keyword_data, time_step, num_predictions):
    # Get the last known data for the keyword
    last_sequence = keyword_data[
        -time_step:
    ]  # The last time-step sequence from the data
    predictions = []
...
def generate_future_dates(last_date, num_predictions, interval_days=7):
    return pd.date_range(
        start=last_date + pd.Timedelta(days=interval_days),
...
def generate_predictions():
    ...
    data = pd.read_csv(
        "s3://mads-siads699-capstone-cloud9/data/combined_trend_data.csv"
    )
    # Import model
    bucket_name = "mads-siads699-capstone-cloud9"
    ...
    for keyword in data.columns[1:]:  # loop through each unique keyword
        keyword_column_index = data.columns.get_loc(
            keyword
        )  # Get the column index for the keyword
        keyword_data = data[keyword].values  # Extract the data for the keyword
        future_predictions = predict_future(
            model, keyword_data, time_step, num_predictions=10
        )

        # add dataframe with future predictions for keyword to list
        predicted_dfs.append(
            pd.DataFrame({"date": new_dates, keyword: future_predictions})
        )
    ...
    merged_predictions_df = merged_predictions_df.loc[
        :, ~merged_predictions_df.columns.duplicated()
    ]
    ...
    combined_trend_data_df = pd.read_csv(
        "s3://mads-siads699-capstone-cloud9/data/combined_trend_data.csv"
    )
...
def generate_seasonal():
    ...
    for word in data.columns[1:]:
        ...
        mask[-max_index] = fft_result[
            -max_index
        ]  # Include the symmetric part for real signals
        ...
        if dominant_freq > 0:
            ...
            if period_in_weeks < 4:
                temp_df_dom_cycle = pd.DataFrame(
                    {
                        "variable": [word],
                ...
                df_fourier_dom_cycle = pd.concat(
                    [df_fourier_dom_cycle, temp_df_dom_cycle]
                )
    ...
    df_fourier_dom_cycle["text"] = df_fourier_dom_cycle.apply(
        lambda x: "The dominant cycle occurs every "
        + str(round(x["value"], 2))
        + " "
        + x["period"],
        axis=1,
